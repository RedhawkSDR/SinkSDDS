...
static connection_t unicast_open_ (const char* iface, const char* group, int port)
{
  unsigned int ii;
...
{
  connection_t client = unicast_open_(iface, group, port);
  return client;
}
...
{
  connection_t server = unicast_open_(iface, group, port);
  if (server.sock != -1) {
    uint8_t ttl = 32;
